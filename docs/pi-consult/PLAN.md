...
This is the canonical plan for a new Pi extension package named `pi-consult`. Implementation is authorized by the user under the boundaries below. The next owner must preserve the decisions and stop when a required gate is not satisfied. The package implementation has not started in the current scoped state.
